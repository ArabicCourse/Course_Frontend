    ...
    useEffect(() => {
        ...
        if (!loggedInUser) {
            ...
            axios.get('http://localhost:5000/api/course_materials')
                .then(response => {
                    console.log("Received sections from backend:", response.data.sections);
                    ...
                    console.log("User auth status:", user.auth);
                    ...
                    if (itemName) {
                        ...
                        if (foundItem) {
                            if (foundItem.authorized && !user.auth) {
                                // Redirect to home if user is not authorized for this item
                                navigate('/payment');
                            } else {
                                setSelectedItem(foundItem);
                            }
                        } else {
                            navigate('/'); // Redirect to home if item not found
                        }
                    }
                })
                .catch(error => {
